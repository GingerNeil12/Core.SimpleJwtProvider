...
namespace SimpleJwtProvider.Extenstions
{
    public static class ServiceCollectionExtension
    {
        ...
        /// <param name="services">IServiceCollection</param>
        /// <param name="options">Confiuration Options for the JWT Token</param>
        /// <param name="responseBody">Custom Response Body to be sent on either Authentication failed or Access Forbidden</param>
        public static void AddJwtAuthentication(this IServiceCollection services,
            JwtConfigurationOptions options, object responseBody)
        {
            services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
                .AddJwtBearer(JwtBearerDefaults.AuthenticationScheme, builder =>
                {
                    ...
                    builder.Events = new JwtBearerEvents()
                    {
                        OnAuthenticationFailed = context =>
                        {
                            var bodyData = ConvertModelForSending(responseBody);
                            if (context.Exception is SecurityTokenExpiredException)
                            {
